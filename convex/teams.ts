...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    let items = await ctx.db.query("teams").collect();

    const nameFilter = normalizeFilter(args.filterName);
    if (nameFilter) {
      items = items.filter((team) => team.name.toLowerCase().includes(nameFilter));
    }
    if (args.filterSport) {
      items = items.filter((team) => team.sport === args.filterSport);
    }
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    const id = await ctx.db.insert("teams", args.data);
...
export const clear = mutation({
  args: {},
  handler: async (ctx) => {
    const items = await ctx.db.query("teams").collect();
    for (const item of items) {
      await ctx.db.delete(item._id as Id<"teams">);
    }
